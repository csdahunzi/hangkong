...
!     s3chkArgsA   s3defaultsA  s3printA   s3sizeA   s3buildA   s3inA
!     s3outA       s3permuteA   s3mapA
!
!     SNOPTB  Basic format
!     s3chkArgsB   s3printB
!
...
!     s3chkArgsN   s3HesN       s3inN   s3outN   s3printN
!
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

      subroutine s3chkArgsA
     ...
     &     none
      integer
     &     Errors, iExit, startType, nF, n, nxname, nFname,
     ...
     &     xnames(nxname)*8, Fnames(nFname)*8
      ...
      !=================================================================
      ! s3chkArgsA   checks the arguments for snOptA.
      !
      ! On exit, Errors says how many errors were encountered.
      ! startType is an integer version of start:
      !    start     startType
      !    'Cold'       0
      !    'Basis'      1
      !    'Warm'       2
      ...
      !
      ! 21 Dec 2002: First version of s3chkArgsA.
      ...
      ! 04 Dec 2004: Current version of s3chkArgsA.
      !=================================================================
      character
     &     str*132
      logical
     ...
     &     b1, b2, infBnd
      ...
      ! The value of the optional parameter takes precedence.

      if (startType .ne. idummy) then ! lvlStart was set as an option
         if (     startType .eq. COLD
     &     .or.   startType .eq. BASIS
     &     .or.   startType .eq. WARM ) then
            ! Relax
         else if (startType .eq. HOT  ) then
            iw(gotFac) = 1
            iw(gotHes) = 1
            iw(gotScl) = 1
         else                      ! lvlStart is an unrecognized option
            startType = idummy
         end if
      end if

      if (startType .eq. idummy) then ! lvlStart is unset
     ...
     &    .or. (nxname .ne. 1  .and.  nFname .eq. 1)) then
      ...
      else if (startType .eq. WARM  .or.  startType .eq. HOT) then
         do j  = 1, n
            js = xstate(j)
      ...
      do j  = 1, n
         b1 = xlow(j)
         b2 = xupp(j)
         ok = b1 .le. b2  .and.  b1 .lt. infBnd  .and.  b2 .gt. -infBnd
 ...
 1610 format(' XXX  Invalid arguments Flow, Fupp: ', i6,
     &       ' inconsistent bounds or infinite equal bounds.')
      ...
      end ! subroutine s3chkArgsA

!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

      subroutine s3defaultsA
     ...
     &     lvlHes, maxR, maxS, mQNmod
      ...
      !=================================================================
      ! s3defaultsA  checks the optional parameters that depend on
      ! dimensions that are not yet known.
      !
      ! The parameters will be checked again later.
      !
      ! See  snworkspace.info  for full da description of iw.
      !
      ! 24 Nov 2007: first version of s3defaultsA.
      !=================================================================
      integer
     &     kreset, never, QPslvr
      !-----------------------------------------------------------------
     ...
     &                        lvlHes = FM
      ...
      end ! subroutine s3defaultsA

!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

      subroutine s3printA
     &   ( m, n, nnCon, nnJac, nnObj, startType, iw, leniw, rw, lenrw )

      implicit
     &     none
      integer
     &     m, n, nnCon, nnJac, nnObj, leniw, lenrw, startType,
     &     iw(leniw)
      double precision
     &     rw(lenrw)

      !=================================================================
      ! s3printA prints the settings of the optional parameters for the
      ! snoptA wrapper.
      !
     ...
     &     lvlSch, lvlScale, lvlSys, lvlTim, lvlVer,
     &     maxmn, maxR, maxS, mflush, minmax, minPrc, mjrPrtlvl,
     &     mMajor, mMinor, mnrPrtlvl, mQNmod, mNewSB, nnL,
     &     nParPrLP, nParPrQP, nPr1, nPr2,
     &     ObjRow, QPslvr, stickyOp
      double precision
     &     bigdx, bigFx,
     &     eps, epsrf, etarg,
     &     fdint1, fdint2, maxTime, scltol, timeLim,
     &     tolCG, tCrash, tolCon, tolFac, tolOptNP, tolpiv, tolOptQP,
     &     tolSwp, tolUpd, tolx, Utol1, viLim, wolfeG,
     &     wtInf0, xdlim, xPen0, zero
      character
     &     str1*132, str2*132, str3*132, str4*132, str5*132
      !-----------------------------------------------------------------
      parameter         (zero   = 0.0d+0)
      !-----------------------------------------------------------------
     ...
     &                           ' Hot start..............'/
      ...
      nParPrLP  = iw( 99) ! # of partial pricing sections
      nParPrQP  = iw(100) ! # of partial pricing sections
      ...
      if (iPrint .le. 0 .or. mjrPrtlvl .eq. 0 .or. lprPrm .eq. 0) return

      nnL = max( nnJac, nnObj )

      if (maxTime  .le. zero) then
         timeLim = 9999999.0d+0
      else
         timeLim = maxTime
      end if
      ...
      write(str3, 2330) lvlScale, tolOptQP, mnrPrtlvl
      write(str4, 2340) tCrash  , tolpiv,   mNewSB
      write(str5, 2350) iCrash  , wtInf0
      call snPRNT( 1, str1, iw, leniw )
      call snPRNT( 1, str2, iw, leniw )
      call snPRNT( 1, str3, iw, leniw )
      call snPRNT( 1, str4, iw, leniw )
      ...
      ! Parital pricing
      !-----------------------
      call snPRNT(11, ' Partial pricing', iw, leniw )
      call snPRNT( 1, ' ---------------', iw, leniw )

      minPrc = 10
      nPr1   = n / nParPrLP
      nPr2   = m / nParPrLP
      if (max( nPr1, nPr2 ) < minPrc) then
         maxmn    = max( m, n )
      ...
      write(str1, 2810) nParPrLP, nPr1, nPr2
      ...
      write(str2, 2820) nParPrQP, nPr1, nPr2
      ...
      call snPRNT( 1, ' --------------', iw, leniw )
      write(str1, 2410) prbtyp(2+minmax),SrtTyp(startType),lvlPPm
      ...
      write(str3, 2470) mMinor, xPen0 , mjrPrtlvl
      write(str4, 2480) timeLim,tolOptNP
      call snPRNT( 1, str1, iw, leniw )
      call snPRNT( 1, str2, iw, leniw )
      call snPRNT( 1, str3, iw, leniw )
      call snPRNT( 1, str4, iw, leniw )
      ...
      write(str4, 2740)                      NoYes(stickyOp)
      call snPRNT( 1, str1, iw, leniw )
      call snPRNT( 1, str2, iw, leniw )
      call snPRNT( 1, str3, iw, leniw )
      call snPRNT( 1, str4, iw, leniw )

      write(str1, 3000) lvlScale
 ...
 2810 format(
     &     ' LP Partial price.......', i10,      6x,
     &     ' Prtl price section ( A)', i10,      6x,
     &     ' Prtl price section (-I)', i10)
 2820 format(
     &     ' QP Partial price.......', i10,      6x,
     &     ' Prtl price section ( A)', i10,      6x,
     &     ' Prtl price section (-I)', i10)

 3000 format(' Scale option', i3 )

      end ! subroutine s3printA
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     m, negCon, neJ, nnCon, nnJac, nnObj, iObj,
     &     kx, leniw, iw )

      implicit
     &     none
      integer
     &     iExit, iObj, lenA, lenG, leniw, m, n, nF, nkx, neA,
     &     neG, negCon, neJ, nnCon, nnJac, nnObj, ObjRow,
     &     iAfun(lenA), jAvar(lenA), iGfun(lenG), jGvar(lenG),
     &     kx(nkx), iw(leniw)

      !=================================================================
      ! s3sizeA runs through the data once in order to compute the
      ...
      !
      !         negCon,  neJ,  nnCon, nnJac,  nnObj  and  iObj.
      !
      ! On entry:
      ! ---------
      ...
      !
      ! 08 Nov 1998: First version of s3sizeA.
     ...
     &       iN .lt. 0  .or.  iN .gt. nF       ) then
         ...
         neJ = neJ + 1    ! All linear terms are held in J.
      ...
      if (ObjRow .eq. 0) then
         !-------------------------------------------
         ! No objective.  SNOPT  will provide one.
         !-------------------------------------------
         if (first) then
            call snPRNT( 13, ' ', iw, leniw )
            first = .false.
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     none
      integer
     &     lenA, lenG, n, nkx, neA, neJ, neG, negCon, nnCon,
     &     nnJac, nlocJ, nlocG, ObjRow, kx(nkx), nGlin(nlocG),
     &     iAfun(lenA), jAvar(lenA), iGfun(lenG), jGvar(lenG),
     &     locG(nlocG), indG(negCon), locJ(nlocJ), indJ(neJ), iy(n)
      double precision
     &     Jcol(neJ), A(lenA)

      !=================================================================
      ! s3buildA  builds the data structure (locJ, indJ, Jcol) for the
      ! full sparse Jacobian.
      !
      ! On entry,
      ...
      !
      ! 25 Sep 1998: First version of s3buildA.
      ! 18 Jul 2000: First version for SNOPT 6.1.
      ! 06 Apr 2008: ObjRow > 0 implies iObj > 0.
      !=================================================================
      integer
     &     i, iN, j, jN, k, nColj, neJJ, nextG, nextJ, Row, Col
      !-----------------------------------------------------------------
      double precision   zero
      parameter         (zero   =  0.0d+0)
      !-----------------------------------------------------------------
      ...
      ! In this case, neJ = 1 was set in s3SizA.
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     xnames, Fnames, Names, nxname, nFname, nNames,
     ...
     &     none
      integer
     &     iObj, m, n, nb, nF, nNames, nnCon, nkx, nxname, nFname,
     ...
     &     Fx(nF), pi(m)
      character
     &     xnames(nxname)*8, Fnames(nFname)*8, Names(nNames)*8
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     rc(nb)
      ...
      ! 17 Jul 2005: fObj assigned to F + x(n+iObj) instead of objTrue.
      ! 06 Apr 2008: ObjRow > 0 implies iObj > 0.
      !=================================================================
      integer
     &     i, j, iN, jN
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     iGfunN(lenG), jGvarN(lenG), iGfun(lenG), jGvar(lenG)
      ...
      !=================================================================
      ! s3permuteA makes a reordered copy of the G coordinate array,
      ! and inverts the row and column permutations.
      !
      ! 29 Dec 2000: First version of s3permuteA.
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     none
      integer
     &     leniw, m, n, neJ, nF, neG, negCon, nextcw, nextiw, nextrw,
     &     nkx, nNames, nnJac, iw(leniw)
     ...
     &     lxN, nb, nlocG, nlocJ
      ...
      iw(269) = lbl       ! bl(nb)   = user-defined lower bounds
      iw(270) = lbu       ! bu(nb)   = user-defined upper-bounds
      ...
      iw(359) = lNames    ! Names(nNames), row and column names

      end ! subroutine s3mapA

!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

      subroutine s3chkArgsB
     &   ( iExit, Start, m, n, neJ, nNames, nS,
     &     nnCon, nnObj, nnJac, iObj,
     &     indJ, locJ, bl, bu, Names,
     &     hs, pi, startType, Errors,
     &     iw, leniw, rw, lenrw )

      implicit
     &     none
      integer
     &     Errors, iExit, iObj, leniw, lenrw, startType,
     &     m, n, neJ, nNames, nS, nnCon,
     &     nnObj, nnJac, indJ(neJ), hs(n+m), locJ(n+1),
     &     iw(leniw)
      double precision
     ...
     &     Names(nNames)*8
      ...
      !=================================================================
      ! s3chkArgsB   checks the arguments for snOpt.
      !
      ! On exit, Errors says how many errors were encountered.
      !
      ! 21 Dec 2002: First version of s3chkArgsB.
      ! 03 Aug 2003: snPRNT adopted.
      ...
      ! 04 Dec 2004: Current version of s3chkArgsB.
      !=================================================================
      character
     &     ch1*1, str*132
      logical
     ...
     &     .or.   startType .eq. WARM ) then
            ...
            iw(gotHes) = 1
            iw(gotScl) = 1
         else                      ! startType is an unrecognized option
            startType  = idummy
         end if
      end if
      ...
      if (neJ .lt. 1   .or.  (k.eq.0 .and. neJ/n1 .gt. m)
     &                 .or.  (k.gt.0 .and. neJ/n1 .ge. m)) then
         Errors = Errors + 1
         if (Errors .le. argerr) then
            write(str, 1100) 'neJ    ', neJ
            call snPRNT( 13, str, iw, leniw )
         end if
      end if
      ...
      if (nNames .ne. 1      .and.  nNames .ne. nb   ) then
         Errors = Errors + 1
         if (Errors .le. argerr) then
            write(str, 1100) 'nNames', nNames
            call snPRNT( 13, str, iw, leniw )
         end if
      end if
     ...
     &    .or.  iObj .lt. 0  .or.   iObj  .gt. m    ) then
      ...
      if (locJ(1) .ne. 1  .or.  locJ(n+1) .ne. neJ+1) then
         Errors = Errors + 1
         if (Errors .le. argerr) then
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     rw(lenrw)
      ...
      !=================================================================
      ! s3printB prints the settings of the optional parameters for the
      ! standard SNOPT wrapper.
      !
      ! See  snworkspace.info  for full documentation of cw, iw and rw.
      !
      ! 15 Nov 1991: First version of s3printB (s8prnt).
      ! 10 Dec 2002: More LU pivoting options.
     ...
     &     lvlSch, lvlScale, lvlSys, lvlTim, lvlVer,
     &     maxmn, maxR, maxS, mflush, minmax, minPrc, mjrPrtlvl, mMajor,
     &     mMinor, mNewSB, mnrPrtlvl, mQNmod, nnL,
     &     nParPrLP, nParPrQP, nPr1, nPr2,
     &     QPslvr, stickyOp
      double precision
     &     bigdx, bigFx,
     &     eps, epsrf, etarg,
     &     fdint1, fdint2, maxTime, scltol, timeLim,
     &     tCrash, tolCon, tolCG, tolFac, tolOptNP, tolpiv, tolOptQP,
     &     tolSwp, tolUpd, tolx, Utol1, viLim, wolfeG,
     &     wtInf0, xdlim, xPen0, zero
      character
     &     str1*132, str2*132, str3*132, str4*132, str5*132
     ...
     &                           ' Hot start..............'/
      ...
      call snPRNT( 1, ' --------------', iw, leniw )
      write(str1, 2410) prbtyp(2+minmax),SrtTyp(startType),lvlPPm
      write(str2, 2420) nnObj , tolOptNP, epsrf
      write(str3, 2430) bigdx , maxS    , fdint1
      write(str4, 2440) bigFx , maxR    , fdint2
      call snPRNT( 1, str1, iw, leniw )
      call snPRNT( 1, str2, iw, leniw )
 ...
 3000 format(' Scale option', i3)
      ...
      end ! subroutine s3printB

!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

      subroutine s3chkArgsQ
     &   ( iExit, Start, m, n, neA, nNames, nS,
     &     lencObj, iObj, ncolH,
     ...
     &     none
      integer
     &     Errors, iExit, iObj, lencObj, leniw, lenrw, startType, m, n,
     &     ncolH, neA, nNames, nS, indA(neA), hs(n+m), locA(n+1),
     &     iw(leniw)
      double precision
     &     bl(n+m), bu(n+m), pi(m), rw(lenrw)
      character*(*)
     ...
     &     Names(nNames)*8
      ...
      !=================================================================
      ! s3chkArgsQ   checks the arguments for sqOpt.
      !
      ! On exit,
      ...
      !    'Hot FHS'   etc.
      !
      ! 18 Oct 2003: First version of s3chkArgsQ.
      ! 08 Mar 2004: Start = 'Hot FHS' options decoded.
      ! 26 Mar 2004: Prevent n*m overflow (noticed by Anders Goran).
      ! 02 Sep 2004: Length of str increased to 132.
      ! 04 Dec 2004: Current version of s3chkArgsQ.
      !=================================================================
      character
     &     ch1*1, str*132
      logical
     ...
     &     .or.   startType .eq. WARM ) then
      ...
      ! 26 Mar 2004: Test if neA > n*m without causing overflow.

      n1 = max(n,1)
      k  = mod(neA,n1)

      if (neA .lt. 1   .or.  (k.eq.0 .and. neA/n1 .gt. m)
     &                 .or.  (k.gt.0 .and. neA/n1 .ge. m)) then
         Errors = Errors + 1
         if (Errors .le. argerr) then
            write(str, 1100) 'neA    ', neA
            call snPRNT( 13, str, iw, leniw )
      ...
      if (locA(1) .ne. 1  .or.  locA(n+1) .ne. neA+1) then
         Errors = Errors + 1
         if (Errors .le. argerr) then
 ...
 1600 format(' XXX  Invalid arguments bl, bu: ', i6,
     &       ' inconsistent bounds or infinite equal bounds.')
      ...
      end ! subroutine s3chkArgsQ

!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

      subroutine s3printQ
     &   ( m, n, ngObj, nnH, startType, iw, leniw, rw, lenrw )

      implicit
     &     none
      integer
     &     m, n, ngObj, nnH, leniw, lenrw, startType, iw(leniw)
      double precision
     &     rw(lenrw)

      !=================================================================
      ! s3printQ prints the settings of the optional parameters for the
      ! standard SNOPT wrapper.
      !
      ! See  snworkspace.info  for full documentation of cw, iw and rw.
      !
      ! 15 Nov 1991: First version of s3printQ (s8prnt).
      ! 10 Dec 2002: More LU pivoting options.
      ! 01 Jul 2003: QN QP options added.
      ! 30 Jul 2003: QN CG options added.
      ! 03 Aug 2003: snPRNT adopted.
     ...
     &     iPnch, iPrint, iStdi, iStdo, iSoln, iSpecs, itnlim,
     &     kchk, kDegen, kFac, klog, ksav, kSumm, eMode, lprDbg,
     &     lprPrm, lvlObjE, lvlPiv,
     &     lvlScale, lvlSys, lvlTim, maxmn, maxR, maxS,
     &     minmax, minPrc, mnrPrtlvl,
     &     nParPrLP, nParPrQP, nPr1, nPr2,
     &     nQP, QPslvr, stickyOp
      double precision
     &     bigdx, eps, etarg, maxTime, scltol,
     &     timeLim, tCrash, tolCG, tolFac, tolpiv, tolOptQP,
     &     tolSwp, tolUpd, tolx, Utol1, wtInf0, zero
      character
     ...
     &                           ' Hot start..............'/
      ...
      if (lprPrm .eq. 0  .or.  mnrPrtlvl .eq. 0) return
      ...
      call snPRNT( 1, ' ----------------', iw, leniw )
      write(str1, 2310) prbtyp(2+minmax),
     &                             QPtype(QPslvr), SrtTyp(startType)
      write(str2, 2320) scltol  ,  tolx    , itnlim
      write(str3, 2330) lvlScale,  tolOptQP, mnrPrtlvl
      write(str4, 2340) tCrash  ,  tolpiv,   eMode
      write(str5, 2350) iCrash  ,  wtInf0,   lvlObjE
      write(str6, 2370) timeLim
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      subroutine s3chkArgsN
     &   ( iExit, Start, ldA, ldcJ, ldH,
     &     n, nclin, ncnln, nNames,
     &     bl, bu, Names, iState, cMul, startType, Errors,
     &     iw, leniw, rw, lenrw )

      implicit
     &     none
      integer
     &     Errors, iExit, ldA, ldcJ, ldH, leniw, lenrw, startType,
     &     n, nclin, ncnln, nNames, iState(n+nclin+ncnln),
     &     iw(leniw)
      double precision
     ...
     &     Names(nNames)*8
      ...
      !=================================================================
      ! s3chkArgsN   checks the arguments for npOpt.
      !
      ! On exit, Errors says how many errors were encountered.
      !
      ! 17 Mar 2002: First version of s3chkArgsN.
      ! 17 Jun 2004: Current version of s3chkArgsN.
      !=================================================================
      character
     &     ch1*1, str*132
      logical
     ...
     &     .or.   startType .eq. WARM ) then
      ...
      if (startType .eq. WARM  .or.  startType .eq. HOT) then
         do j  = 1, nb
            iS = iState(j)
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     nnCon0, nnCon, neJ, nlocJ, leniw, lenrw, indJ(neJ), hs(nb),
     &     iState(n+nCon), locJ(nlocJ), iw(leniw)
      double precision
     &     Alin(ldA,*), bl(n+nCon), bu(n+nCon), bbl(nb), bbu(nb),
     &     c(nnCon0), cMul(n+nCon), Hess(ldH,*), Jcol(neJ),
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     rw(lenrw)
      ...
      !=================================================================
      ! s3printN  prints  x,  A*x, c(x), the bounds, the
      ! multipliers, and the slacks (distance to the nearer bound).
      !
      ! 22 Mar 1997: First version of s3printN.
      ! 03 Aug 2003: snPRNT adopted.
      ! 22 Nov 2003: Current version of s3printN.
      !=================================================================
      external
     &     ddot
      integer
