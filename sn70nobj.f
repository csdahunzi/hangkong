...
*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     neH, neJ, nnL, nnCon0, nnCon, nnJac, nnObj0, nnObj,
     &     nlocG, nlocH, nlocJ, locG(nlocG), locH(nlocH), locJ(nlocJ),
     &     indH(neH), indJ(neJ), iu(leniu), iw(leniw)
      double precision
     &     bl(nnL), bu(nnL), fObj,
     &     gObj (nnObj0), fCon (nnCon0), gCon (negCon),
     &     gObj2(nnObj0), fCon2(nnCon0), gCon2(negCon), Hcol(neH),
     &     x(n), x1(n), yCon(nnCon0), y(nnL), y1(nnCon0), ru(lenru),
     &     rw(lenrw)
      character
...
*     ==================================================================
     ...
     &     key*4, str*120
      logical
     &     Cheap, Done, First, Found, NonlinearCon, NonlinearObj
      integer
...
*     ------------------------------------------------------------------
      ...
      do j  = 1, nnL
         ...
         NonlinearObj = j .le. nnObj .and.jN .ge. j1 .and. jN .le. j2
         NonlinearCon = j .le. nnJac .and.jN .ge. j3 .and. jN .le. j4

         if (NonlinearCon  .or.  NonlinearObj) then
...
*     ------------------------------------------------------------------
 ...
 2702 format(' -->  The largest relative error was', 1p, e12.2,
     &       '   in column', i6)
      ...
      end ! subroutine s7checkG

!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

      subroutine s7checkHv
     &   ( iExit,
     &     n, nnCon0, nnCon, nnJac, nnH, nnObj0, nnObj,
     &     funwrapper, funcon, funobj, userHv,
     &     x, x1, bl, bu,
     &     neJ   , nlocJ, locJ, indJ,
     &     negCon, nlocG, locG,
     &     neH   , nlocH, locH, indH, Hcol, Hv,
     &     gCon, gObj, fCon2, gCon2, gObj2,
     &     yCon, w, w1,
     &     cu, lencu, iu, leniu, ru, lenru,
     &     cw, lencw, iw, leniw, rw, lenrw )
     ...
     &     funwrapper, funcon, funobj, userHv
      integer
     &     iExit, lencu, leniu, lenru, lencw, leniw, lenrw, n, neJ,
     &     negCon, neH, nnH, nnCon0, nnCon, nnJac, nnObj0, nnObj, nlocG,
     &     nlocH, nlocJ, locG(nlocG), locH(nlocH), locJ(nlocJ),
     &     indH(neH), indJ(neJ), iu(leniu), iw(leniw)
      double precision
     &     bl(nnH), bu(nnH), gObj(nnObj0), gCon(negCon),
     &     gObj2(nnObj0), gCon2(negCon), fCon2(nnCon0), Hcol(neH),
     &     Hv(nnH), yCon(nnCon0), x(n), x1(n), w(nnH), w1(nnCon0),
     &     ru(lenru), rw(lenrw)
      character
     &     cu(lencu)*8, cw(lencw)*8
      ...
      !=================================================================
      external
     &     idamax, dasum, dnormi, s2ColN
      character
     &     key*4, str*120
      logical
     &     Cheap, First, NonlinearCon, NonlinearObj
      integer
     &     i, irN, idamax, imaxH, jmaxH, j, j5, j6, jN, lvlVer,
     &     funmode, nFeas, nzero, RightH, WrongH, s2ColN, status
      double precision
     &     abHij, dx, emaxH, eps0, eps5, err, fdint1, fObj2, Hij, HijFD,
     &     maxH, xj, wj, dasum, dnormi
      !-----------------------------------------------------------------
      integer            Transp
      parameter         (Transp = 1)
      double precision   zero,          one,          ok
      ...
      j5        = iw(114) ! start col for Hessian checking
      j6        = iw(115) ! stop  col for Hessian checking
      ...
      if (Cheap) then
         call snPRNT( 11,
     &      ' Cheap test of user-supplied  Lagrangian Hessian...',
     &      iw, leniw )
      else
         call snPRNT( 11,
     &      ' Verification of user-supplied second derivatives.',
     &      iw, leniw )
      end if
      ...
      dx     = fdint1 * (one + dnormi( nnH, x, 1 ))
      call s7checkp
     &   ( nnH, bl, bu, x, dx, w, nfeas )
      if (nfeas .eq. 0)
     &call dcopy ( nnH, x1, 1, w, 1 )

      !-----------------------------------------------------------------
      ! Compute functions at a short step along  w.
      !-----------------------------------------------------------------
      dx     = fdint1 * (one + dasum( nnH, x, 1 ))
      call dcopy ( nnH,     x, 1, x1, 1 )
      call daxpy ( nnH, dx, w, 1, x1, 1 )
      ...
      call funwrapper
     &   ( iExit,
     &     funmode, NonlinearCon, NonlinearObj,
     &     n, negCon, nnCon0, nnCon,
     ...
     &     neJ, nlocJ, locJ, indJ,
     &     neH, nlocH, locH, indH, Hcol,
     &     fCon2, fObj2, gCon2, gObj2, yCon,
     &     cu, lencu, iu, leniu, ru, lenru,
     &     cw, lencw, iw, leniw, rw, lenrw )
      if (iExit .ne. 0) go to 900

      if (nnObj .gt. 0) then ! Compute gL2 - gL
         call dcopy ( nnObj,         gObj2, 1, w1, 1 )
         call daxpy ( nnObj, (-one), gObj , 1, w1, 1 )
      end if

      nzero = nnH - nnObj
      if (nzero .gt. 0) call dload ( nzero, zero, w1(nnObj+1), 1 )

      if (nnCon  .gt. 0) then
         call s8Gprod
     &      ( Transp, eps0,
     &        neJ, nlocJ, locJ, indJ, negCon, nlocG, locG, gCon2,
     &        (-one), yCon, nnCon, one, w1, nnJac )
         call s8Gprod
     &      ( Transp, eps0,
     ...
     &     cw, lencw, iw, leniw, rw, lenrw )
      ...
      jmaxH  = idamax( nnH, Hv, 1 )
      maxH   = w1(jmaxH)/dx
      emaxH  = abs( Hv(jmaxH) )/(one + abs( maxH ))
      if (emaxH .le. ok) then
         call snPRNT( 11,
     &      ' The Lagrangian Hessian seems to be OK.',
     &        iw, leniw )
      else
         call snPRNT( 11,
     &      ' XXX  The Lagrangian Hessian seems to be incorrect.',
     &        iw, leniw )
      end if

      write(str, 1600) emaxH, s2ColN( jmaxH, leniw, iw )
      call snPRNT( 11, str, iw, leniw )
      call snPRNT(  1, ' ', iw, leniw )
      ...
      do j = 1, nnH
         ...
         if (jN .ge. j5  .and.  jN .le. j6) then
            ...
            if (bl(j) .lt. bu(j)  .and.  xj .ge. bu(j)) then
               dx   = -dx
            end if
            ...
            funmode = 2
            call funwrapper
     &         ( iExit, funmode, NonlinearCon, NonlinearObj,
     &           n, negCon, nnCon0, nnCon, nnJac, nnH, nnObj0, nnObj,
     &           funcon, funobj, userHv,
     &           x, yCon,
     &           neJ, nlocJ, locJ, indJ,
     &           neH, nlocH, locH, indH, Hcol,
     ...
     &           cw, lencw, iw, leniw, rw, lenrw )
            ...
            do i = 1, nnH
               irN   = s2ColN( i, leniw, iw )

               Hij   = Hv(i)
               HijFD = w1(i)
               abHij = abs(Hij)
               err   = abs(HijFD - Hij)/(one + abHij)

               if (emaxH .lt. err) then
                  emaxH  = err
                  imaxH  = irN
                  jmaxH  = jN
               end if
     ...
     &                    irN, Hij, HijFD, key
      ...
      if (j5 .le. j6) then
         if (WrongH .eq. 0) then
            write(str, 2501) RightH, j5, j6
         else
            write(str, 2601) WrongH, j5, j6
         end if
         call snPRNT( 11, str, iw, leniw )
         write(str, 2701) emaxH, imaxH, jmaxH
         call snPRNT( 11, str, iw, leniw )
         call snPRNT(  1, ' ', iw, leniw )

         if (emaxH .ge. one) then
            iExit = 54          ! Bad second derivatives.
         end if
      end if
  ...
  900 if (iExit .lt. 0) then
         call snPRNT( 11,
     &      ' XXX  Unable to complete Hessian check.',
     &              iw, leniw )
         iExit = 0
      end if
 ...
 1600 format(' -->  The largest discrepancy was', 1p, e12.2,
     &           '  in row and column', i6)
 2001 format(' Column       x(j)        dx(j)', 3x,
     &   '     Row        Derivative    Difference approxn')
 2101 format(i7, 1p, e16.8, e10.2, i10,               2e18.8, 2x, a4)
 2201 format(33x, i10, 1pe18.8, e18.8, 2x, a4)
 2501 format(' All', i7, '  nonzero Hessian elements in cols ', i6,
     &       '  thru', i6, '  seem to be OK.')
 2601 format(' XXX  There seem to be', i6,
     &       '  incorrect Hessian elements in cols', i6, '  thru', i6)
 2701 format(' -->  The largest relative error was', 1p, e12.2,
     &       '   in row', i6, ',  column', i6)
...
*           Give up and use set pj = zero if both bounds are too close.
      ...
      end ! subroutine s7checkp

*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

      subroutine s7fixX
     &   ( n, nFixed, PerturbAll, boundPert, bl, bu, x )
...
*     ------------------------------------------------------------------
      ...
      do j = 1, n
         ...
         if (bl(j) .eq. bu(j)) then
            nFixed = nFixed + 1
            if ( PerturbAll ) then
               bl(j) = bl(j) - boundPert*absxj
               bu(j) = bu(j) + boundPert*absxj
            end if
         end if
      end do
...
*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     iGfun(lenG), jGvar(lenG), rowType(nF), colType(n), iw(leniw),
     &     iu(leniu)
      double precision
...
*     If requested, perturb bounds for fixed variables.
      ...
      call s7fixX
     &   ( n, nFixed, PerturbAll, one, xlow, xupp, x )
...
*     Create a random feasible perturbation of the variables.
     ...
     &   ( n, seeds, x, xlow, xupp, xnorm, z )
      ...
      call iload ( n , Linear, colType, 1 )
      call iload ( nF, Linear, rowType, 1 )
...
*     -----------------------------------------------------------------
      ...
      do k0 = 1, neA0
         i  = iAfun(k0)
         j  = jAvar(k0)
         if (rowType(i) .eq. Nonlin  .and.  colType(j) .eq. Nonlin) then
            neA = neA - 1
            neG = neG + 1
            if (neG .gt. lenG) then
               call snPRNT( 1, ' Increase lenG.', iw, leniw )
