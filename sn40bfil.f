...
!     File  sn40bfil.f
!
!     s4getB     s4id     s4ksave  s4name   s4insert  s4load    s4oldB
!     s4checkhs  s4chkP   s4dump   s4newB   s4punch   s4report  s4saveB
!     s4soln     s4solp   s4stat
!
!     09 Mar 2004: snSolF implemented and called by s4soln.
!     17 Jun 2004: s4saveB saves biggest primal and dual infeasibilities
!                  before and after scaling, so GAMS can use them.
!     27 Dec 2013: s4saveB unscales gObj even for Scale option 1.
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     none
      integer
     &     iExit, iObj, m, n, nb, nNames, nS, leniw, lenrw, hs(nb),
     &     iw(leniw)
      double precision
...
!     If nNames = nb, the name is already in Names.
!     Otherwise nNames = 1. Some generic column or row name is cooked up
!
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     scales, bl, bu, x, xBS, cw, lencw, iw, leniw )

      implicit
     &     none
      integer
     &     minimize, m, n, nb, nS, mBS, itn, nInf, lencw, leniw,
     &     hs(nb), kBS(mBS), iw(leniw)
      double precision
     &     sInf, f, scales(nb), bl(nb), bu(nb), xBS(mBS), x(nb)
      character
     &     cw(lencw)*8

!     ==================================================================
!     s4ksave  saves various quantities as determined by the frequency
!     control ksav.
!
!     15 Nov 1991: First version.
!     07 May 2006: s4newB handles negative values of hs in situ.
!     ==================================================================
      character
     &     istate(3)*4
      integer
...
!     ------------------------------------------------------------------
     ...
     &      ( k, istate )
         call s4newB
     &      ( Freq, iNewB, minimize, m, n, nb,
     &        nS, mBS, itn, nInf, sInf, f, kBS, hs,
     &        scales, bl, bu, x, xBS, istate,
     &        cw, lencw, iw, leniw )

         if (iBack .gt. 0)
     &   call s4newB
     &      ( Freq, iBack, minimize, m, n, nb,
     &        nS, mBS, itn, nInf, sInf, f, kBS, hs,
     &        scales, bl, bu, x, xBS, istate,
     &        cw, lencw, iw, leniw )
      end if

      end ! subroutine s4ksave
...
!     Used by subroutines s3mpsc, s4insert, s4load.
!
...
!     ==================================================================
!     This impression of INSERT reads a file produced by  s4punch.
...
!     01 Aug 2003: snPRNT adopted.
!     08 Oct 2003: Current version of s4insert.
!     ==================================================================
      character
...
!     ==================================================================
!     s4checkhs  takes hs and x and checks they contain reasonable values.
...
!
!     15 Nov 1991: First version (s4check) based on Minos subr m4chek.
!     01 Aug 2003: snPRNT adopted.
!     08 Mar 2004: NeedB, gotHes, SetkS allow for Hot start.
!     23 Feb 2015: Name changed to s4checkhs.
!     ==================================================================
      character
     &     str*80
      logical
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     none
      integer
     &     iDump, m, n, nb, nNames, lencw, leniw, hs(nb), iw(leniw)
      double precision
     &     x(nb)
      character
...
!     01 Aug 2003: snPRNT adopted.
!     17 Jul 2005: Dump default names when nNames = 1
!     ==================================================================
      integer
     &     iPrint, j, k
      character
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     none
      integer
     &     job, iNewB, minimize, m, n, nb, nS, mBS, itn, nInf,
...
!     ==================================================================
     ...
     &     Scaled
      integer
     &     buffer(80), i, iPrint, j, j1, j2, js, k, lvlScale, nnJac
      double precision
     &     Obj, xj
!     ------------------------------------------------------------------
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      subroutine s4punch
     &   ( iPunch, m, n, nb, nNames,
     &     hs, bl, bu, x, Names, cw, lencw, iw, leniw )

      implicit
     &     none
      integer
     &     iPunch, m, n, nb, nNames, lencw, leniw, hs(nb), iw(leniw)
      double precision
     &     bl(nb), bu(nb), x(nb)
      character
     &     Names(nNames)*8, cw(lencw)*8
...
!     ==================================================================
!     s4punch  outputs a PUNCH file (list of basis names, states and
!     values) in a format that is compatible with MPS/360.
!
!     15 Nov 1991: First version based on Minos 5.4 routine m4punch.
!     16 Jan 2003: Allow for missing names.
!     01 Aug 2003: snPRNT adopted.
!     01 Aug 2003: Current version of s4punch.
!     ==================================================================
      integer
     &     iPrint, irow, j, jN, k
      character
...
!     ------------------------------------------------------------------
      ...
      write(iPunch, 2000) mProb
      irow      = n

      do 500, j = 1, n
         jN     = j
         call s4id
     &      ( jN, n, nb, nNames, Names, ColNm )
         k       = hs(j)
...
!           Basics -- find the next row that isn't basic.
  ...
  300       irow   = irow + 1
            if (irow .le. nb) then
               ...
               call s4id
     &              ( irow, n, nb, nNames, Names, Name )
               if (k .eq. 2) k = 0
               write(iPunch, 2100) key(k+4), ColNm, Name, x(j)
            end if
         else
...
!           Skip nonbasic variables with zero lower bounds.
            ...
            write(iPunch, 2100) key(k+1), ColNm, lblank, x(j)
         end if
  500 continue
...
!     Output superbasic slacks.
      ...
      do j = n+1, nb
         if (hs(j) .eq. 2) then
            jN   = j
            call s4id
     &           ( jN, n, nb, nNames, Names, Name )
            write(iPunch, 2100) key(3), Name, lblank, x(j)
         end if
      end do

      write(iPunch , 2200)
      write(str, 3000) iPunch
      call snPRNT( 13, str, iw, leniw )
      if (iPunch .ne. iPrint) rewind iPunch
      return
 ...
 3000 format(' PUNCH file saved on file', i4)
      ...
      end ! subroutine s4punch

!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

      subroutine s4report
     &   ( ondisk, m, n, nb, nNames,
     &     nnCon0, nnCon, nnObj0, nnObj, nS,
     &     neA, nlocA, locA, indA, Acol,
     &     hs, scales, bl, bu, gObj, pi, x, Fx,
     ...
     &     Ondisk
      integer
     &     nNames, m, n, nb, neA, nlocA, nnCon0, nnCon, nnObj0, nnObj,
     &     nS, leniw, indA(neA), hs(nb), locA(nlocA), iw(leniw)
      double precision
     &     Acol(neA), scales(nb), bl(nb), bu(nb), gObj(nnObj0), pi(m),
     &     x(nb), Fx(nnCon0)
      character
     &     Names(nNames)*8, istate(3)*4

!     ==================================================================
!     s4report  has the same parameter list as s4soln, the routine that
...
!
!     This version of s4report does nothing.    Added for PILOT, Oct 1985.
!
!     15 Nov 1991: First version based on Minos 5.4 routine m4rept.
!     26 Mar 2000: Updated for SNOPT 6.1.
!     01 Aug 2003: snPRNT adopted.
!     01 Aug 2003: Current version of s4report.
!     ==================================================================
      character
     &     str*80
!     ------------------------------------------------------------------
 ...
 1000 format(' XXX Report file requested.  s4report does nothing.')

      end ! subroutine s4report

!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

      subroutine s4saveB
     &   ( iExit, Task, minimize, m, n, nb, nkx,
     &     nnCon0, nnCon, nnObj0, nnObj, nNames, nS,
     &     itn, nInf, sInf, wtInf, vimax, iObj, scaleObj, objTrue,
     &     pNorm1, pNorm2, piNorm, xNorm,
     &     neJ, nlocJ, locJ, indJ, Jcol, kx,
     &     eState, hs, scales, bl, bu, Fx, gObj,
     &     Names, pi, rc, x,
     ...
     &     none
      integer
     &     Task, iExit, minimize, itn, iObj, nInf, m, n,
     &     nb, neJ, nkx, nlocJ, nnCon0, nnCon, nnObj0, nnObj, nNames,
     &     nS, lencw, leniw, lenrw, kx(nkx), locJ(nlocJ),
     &     indJ(neJ), eState(nb), hs(nb), iw(leniw)
      double precision
     &     objTrue, pNorm1, pNorm2, piNorm, sInf, scaleObj,
     &     vimax, wtInf, xNorm,
     &     Jcol(neJ), scales(nb), bl(nb), bu(nb), Fx(nnCon0),
     &     gObj(nnObj0), rc(nb), x(nb), pi(m), rw(lenrw)
      character
     &     Names(nNames)*8, cw(lencw)*8

!     ==================================================================
!     s4saveB  saves basis files  and/or  prints the solution.
!     It is called twice at the end of s5solve
!              and twice at the end of s8solve.
!
...
!     26 Mar 2000: Updated for SNOPT 6.1.
!     16 Jan 2003: Reinstated calls to s4dump, s4punch.
...
!     ==================================================================
     ...
     &     Feasible, Prnt
      integer
     &     iDump, iPunch , iPrint, iReport,
     &     iSoln, lvlScale, lprSol, jbInf, jbInf1,
...
!        Find the maximum bound and dual infeasibilities.
         ...
         call s2rcA
     &      ( Feasible, tolx, iObj, minimize, wtInf,
     &        m, n, nb, nnObj0, nnObj, neJ, nlocJ, locJ, indJ, Jcol,
     &        eState, hs, bl, bu, gObj, pi, rc, x )
         call s2bInf
     ...
     &      ( n, nb, iObj, eps0, bl, bu, rc, x, dInf, jdInf )
         ...
         jbInf1 = jbInf
         jdInf1 = jdInf
         rw(427)=  bInf ! Largest bound infeasibility (  scaled)
         rw(428)=  dInf ! Largest dual  infeasibility (  scaled)
         iw(427)= jbInf
         iw(428)= jdInf
...
!        (Previously, s4soln used the scaled piNorm, but no more.)
         ...
         if (lvlScale .gt. 0) then
            call s2ApplyScales
     &         ( UnScal, m, n, nb, iObj, infBnd, scaleObj,
     ...
     &         ( nb , scales, 1, rc, 1 )
            ...
            !if (lvlScale .eq. 2) then
               if (nnCon .gt. 0)
     &         call ddscl ( nnCon, scales(n+1), 1, Fx  , 1 )
               if (nnObj .gt. 0)
     &         call dddiv ( nnObj, scales     , 1, gObj, 1 )
            !end if
     ...
     &         ( n, nb, iObj, eps0, bl, bu, rc, x, dInf, jdInf )
         ...
         rw(429) =  bInf  ! Largest bound infeasibility (unscaled)
         rw(430) =  dInf  ! Largest dual  infeasibility (unscaled)
         iw(429) = jbInf
         iw(430) = jdInf
...
!        Change the sign of pi and rc if feasible and maximizing.
         ...
         if (nInf .eq. 0  .and.  minimize .lt. 0) then
...
!        Output Punch, Dump, Solution and/or Report files.
     ...
     &           hs, x, names, cw, lencw, iw, leniw )
         ...
         if (iSoln .gt. 0) then
            call s4soln
     &         ( .true., minimize, m, n, nb, nkx, nNames,
     &           nnCon0, nnCon, nnObj0, nnObj, nS, iObj,
     &           itn, nInf, sInf, objTrue, piNorm, Names,
     &           neJ, nlocJ, locJ, indJ, Jcol, kx,
     &           hs, scales, bl, bu,
     ...
     &           cw, lencw, iw, leniw, rw, lenrw )
         ...
         if (iReport .ge. 91  .and.  iReport .le. 93) then
            call s2xmat
     &         ( iReport, n, nb,
     &           neJ, nlocJ, locJ, indJ, Jcol, hs )
         else if (iReport .gt. 0) then
            call s4report
     &         ( .true., m, n, nb, nNames,
     &           nnCon0, nnCon, nnObj0, nnObj, nS,
     &           neJ, nlocJ, locJ, indJ, Jcol,
...
!        ---------------------------------------------------------------
     ...
     &       (lprSol .eq. 3  .and.  iExit .le. 2)) Prnt = .false.
         ...
         if (Prnt) then
            ...
            ! piNorm = pNorm1
            call s4soln
     &         ( .false., minimize, m, n, nb, nkx, nNames,
     &           nnCon0, nnCon, nnObj0, nnObj, nS, iObj,
     &           itn, nInf, sInf, objTrue, piNorm, Names,
     &           neJ, nlocJ, locJ, indJ, Jcol, kx,
     &           hs, scales, bl, bu,
 ...
 1300 format(' Solution not printed')
      ...
      end ! subroutine s4saveB

!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

      subroutine s4soln
     &   ( Ondisk, minimize, m, n, nb, nkx, nNames,
     &     nnCon0, nnCon, nnObj0, nnObj, nS, iObj,
     &     itn, nInf, sInf, ObjAll, piNorm, Names,
     &     neA, nlocA, locA, indA, Acol, kx,
     &     hs, scales, bl, bu,
     &     gObj, pi, rc, x, Fx, istate,
     &     cw, lencw, iw, leniw, rw, lenrw )
     ...
     &     Ondisk
      integer
     &     minimize, nInf, itn, m, n, nb, neA, nkx, nlocA, nNames,
     &     nnCon0, nnCon, nnObj0, nnObj, nS, iObj, lencw, leniw, lenrw,
     &     kx(nkx), indA(neA), hs(nb), locA(nlocA), iw(leniw)
      double precision
     &     ObjAll, piNorm, sInf,
     &     Acol(neA), scales(nb), bl(nb), bu(nb), x(nb), gObj(nnObj0),
     &     rc(nb), pi(m), Fx(nnCon0), rw(lenrw)
      character
     &     istate(3)*4, Names(nNames)*8, cw(lencw)*8
...
!                  scales(*) is no longer referenced.
...
!!    scale     = one
      Feasible  = nInf     .eq. 0
!     maximize  = minimize .lt. 0
!!    scaled    = lvlScale .gt. 0

      call s1page( 1, iw, leniw )

      if (Feasible) then
         write(str, 1002) mProb, ObjAll
         if (Ondisk) then
      ...
      if (mObj .ne. cdummy) then
         write(str, 1005) mObj, Objtyp(minimize+2)(1:3)
         if (Ondisk) then
            call s1trim( str, length )
            write(iSoln, form2) str(1:length)
         else
...
!                  d1, d2, djtest are no longer used here.
