...
!     File  sn20amat.f
!
!     s2Amat   s2Aprod  s2Bprod      s2bInf         s2ColN    s2RowN
!     s2VarN   s2dInf   s2gather     s2scatter      s2crash   s2Mem0
!     s2Mem    s2rcA    s2getScales  s2applyScales  s2unpack
!     s2vmax   s2dmat   s2xmat
!
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

      subroutine s2Amat
     &   ( Task, lPrint, m, n, nb,
     &     nnCon, nnJac, nnObjU, iObj, numLC, numLIQ,
     &     neA, nlocA, locA, indA, Acol,
     &     bl, bu, rowType, iw, leniw, rw, lenrw )

      implicit
     &     none
      integer
     &     iObj, lPrint, leniw, lenrw, m, n, nb, neA, nlocA,
     &     nnCon, nnJac, nnObjU, numLC, numLIQ,
     &     Task, rowType(m), locA(nlocA), indA(neA), iw(leniw)
      double precision
     &     Acol(neA), bl(nb), bu(nb), rw(lenrw)

!     ==================================================================
!     s2Amat defines rowType, the set of row types.
!
...
!        rowType(i) = 0  for E rows      (equalities)
!        rowType(i) = 1  for L or G rows (inequalities)
!        rowType(i) = 2  for N rows      (free rows)
!     They are used in s2getScales and s2crash.
!
...
!     23 Jun 2014: Safeguard Adnsty = 100.0d+0*ne / (m*n) from overflow.
!     ==================================================================
      character
     &     str*80
      logical
...
!     ------------------------------------------------------------------
     ...
     &           // '        Free       Fixed     Bounded', iw, leniw )
          ...
          ! Adnsty = 100.0d+0*ne / (m*n)
            Adnsty = (100.0e+0 * real(neA)) / real(m) / real(n)
            write(str, 2400) neA, Adnsty
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     none
      integer
     &     Task, neA, nlocA, lenx, leny, locA(nlocA), indA(neA)
      double precision
     &     tolz, alpha, beta, Acol(neA), x(lenx), y(leny)

!     ==================================================================
!     s2Aprod computes matrix-vector products involving  x  and a sparse
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     none
      integer
     &     Task, n, neA, nlocA, lenkBS, lenx, leny,
     &     locA(nlocA), indA(neA), kBS(lenkBS)
      double precision
     &     alpha, beta, tolz, Acol(neA), x(lenx), y(leny)

!     ==================================================================
!     s2Bprod computes various matrix-vector products involving
...
!     A = ( B  S ).
!
!     23 Nov 1991: First version of s2Bprod.
...
!     s2dInf  is called by s4saveB before and after unscaling.
!
!     On exit,
...
!     ==================================================================
!     s2gather   performs the gather operation alpha*g  --> gBS
...
!     On entry: 0 < nBS <= neg.
!
!     20 Jul 2000: First version of s2gather
...
!     ------------------------------------------------------------------
      ...
      end ! subroutine s2gather

!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

      subroutine s2scatter
...
!     ==================================================================
!     s2scatter   performs the scatter operation alpha*gBS  --> g
...
!     On entry: 0 < nBS <= neg.
!
!     20 Jul 2000: First version of s2scatter
!     19 Dec 2000: Current version.
!     ==================================================================
      integer
     &     j, k
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     iCrash, tCrash,
     &     neA, nlocA, locA, indA, Acol,
     &     hpiv, hs, rowType, bl, bu, x,
     &     iw, leniw, rw, lenrw )

      implicit
     &     none
      integer
     &     m, n, nb, nnCon, neA, iCrash, nlocA, leniw, lenrw,
     &     locA(nlocA), indA(neA), hpiv(m), hs(nb), rowType(m),
     &     iw(leniw)
      double precision
     &     Acol(neA), bl(nb), bu(nb), x(nb), rw(lenrw)

!     ==================================================================
!     s2crash  looks for a basis in the columns of ( A  -I ).
!
!     ON ENTRY
!
...
!
!     rowType(*)   should be defined as described in s2Amat:
!     rowType(i) = 0  for E rows      (equalities)
!     rowType(i) = 1  for L or G rows (inequalities)
!     rowType(i) = 2  for N rows      (objective or free rows)
!
...
!
!     Stage 1: Insert any slacks (N, L or G rows, rowType = 1 or 2).
!
...
!                 by s4checkhs and variable j will be nonbasic.
!
...
!     31 Jul 2003: snPRNT adopted.
!     31 Jul 2003: Current version of s2crash.
!     ==================================================================
      character
     &     str*80
      logical
...
!     ------------------------------------------------------------------
!     Stage 1: Insert relevant slacks (N, L or G rows, rowType 1 or 2).
...
!     ------------------------------------------------------------------
      ...
      do i = i1, i2
         j = n + i
         if (hs(j) .le. 1  .and.  rowType(i) .gt. 0) then
            addslk = .true.
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      subroutine s2rcA
     &   ( Feasible, featol, iObj, minimize, wtInf,
     &     m, n, nb, leng, neg,
     &     neA, nlocA, locA, indA, Acol,
     &     eState, hs, bl, bu, g, pi, rc, x )
     ...
     &     Feasible
      integer
     &     iObj, minimize, m, n, nb, neA, nlocA, leng, neg,
     &     eState(nb), hs(nb), locA(nlocA), indA(neA)
      double precision
     &     featol, wtInf,
     &     Acol(neA), bl(nb), bu(nb), g(leng), rc(nb), x(nb), pi(m)
...
!
!     s2rcA  is called by s4saveB and s8SQP for scaled data.
...
!        Include the gradient of the linear term.
         ...
         if (neg .gt. 0) then
            call daxpy ( neg, signObj, g, 1, rc, 1 )
         end if

         if (iObj .gt. 0) then
            jObj     = n + iObj
            rc(jObj) = rc(jObj) + signObj
         end if
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      subroutine s2getScales
     &   ( lPrint, m, n, nb, nnL, nnCon, nnJac, rowType,
     &     neA, nlocA, locA, indA, Acol,
     &     scales, bl, bu, rmin, rmax,
     &     iw, leniw, rw, lenrw )

      implicit
     &     none
      integer
     &     m, n, nb, nlocA, nnL, nnCon, nnJac, neA, leniw, lenrw,
     &     rowType(m), locA(nlocA), indA(neA), iw(leniw)
      double precision
     &     Acol(neA), scales(nb), bl(nb), bu(nb), rmin(m), rmax(m),
     &     rw(lenrw)

!     ==================================================================
!     s2getScales computes scale factors  scales  from  A, bl, bu.
!
...
!
!        Free rows (rowType=2) and fixed columns (bl=bu) are not used
!        at this stage.
!
...
!
!
!     If lvlScale = 1, the first nnCon rows and the first nnJac columns
!     will retain scales of 1.0 during phases 1-2, and phase 3 will not
!     be performed.  (This takes effect if the problem is nonlinear but
...
!
!     If lvlScale = 2, all rows and columns are scaled. To guard against
...
!
!     The work vector of row-types is defined as follows:
!        rowType(i) = 0  for E rows      (equalities)
!        rowType(i) = 1  for L or G rows (inequalities "le" or "ge")
!        rowType(i) = 2  for N rows      (free rows)
!
!     15 Nov 1991: First version based on Minos 5.4 routine m2scal.
!     24 Jan 2001: Don't scale BIG free rows anymore!
!     31 Jul 2003: snPRNT adopted.
!     13 Nov 2014: Columns nnJac+1:n are scaled.
!     ==================================================================
      external
     &     dnormi
      character
     ...
     &     jStart, l, lPrint, lprScl, lvlScale, mxPass, nclose, npass
      double precision
...
!     ------------------------------------------------------------------
     ...
     &        // 'Min elem    Max elem       Max col ratio', iw, leniw )
      ...
      Aratio     = bplus
      mxPass     = 10
      JustLinear = lvlScale .eq. 1

      call dload ( nb, one, scales, 1 )

      if (JustLinear) then
         if (nnJac .ge. n) return
         iStart = nnCon + 1
         jStart = nnJac + 1
      else
         iStart = 1
...
!        rowType(i) = 2       for type N rows (objective or free rows),
!        rowType(i) = 0 or 1  otherwise.
!     ------------------------------------------------------------------
      do nPass = 0, mxPass
...
!        Also set new column scales (except on pass 0).
         ...
         do j = jStart, n
            if (bl(j) .lt. bu(j)) then
               ...
               if (nPass .gt. 0    ) scales(j) = Ac
               Amin   = min( Amin, cmin / scales(j) )
               Amax   = max( Amax, cmax / scales(j) )
               cratio = cmax / cmin
...
!        to adjust both row and column scales.
         ...
         Ac     = dnormi( m, rmax, 1 )
         call dddiv ( m, scales(n+1), 1, rmax, 1 )
...
!        problem is unscaled.  If any are too big, scale them down.
         ...
         Amax = zero
         do j = 1, n
            Amax = max( Amax, scales(j) )
         end do

         do i = 1, m
            if (rowType(i) .ne. 2) Amax = max( Amax, scales(n+i) )
         end do
 ...
 1500 format(i6, g16.5)
      ...
      end ! subroutine s2getScales

!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

      subroutine s2applyScales
     &   ( Task, m, n, nb, iObj, infBnd, scaleObj,
     &     neA, nlocA, locA, indA, Acol,
     &     scales, bl, bu, pi, x )
     ...
     &     none
      integer
     &     Task, m, n, nb, iObj, neA, nlocA, locA(nlocA), indA(neA)
      double precision
     &     scaleObj, infBnd,
     &     Acol(neA), scales(nb), bl(nb), bu(nb), x(nb), pi(m)

!     ==================================================================
!     s2applyScales scales or unscales A, bl, bu, pi, x using row and column
!     scales Sr and Sc stored as  scales = ( Sc  Sr ).
!
...
!     24 Mar 2000: Current version of s2applyScales
!     ==================================================================
      integer
     &     i, j, l
      double precision
...
!        ---------------------------------------------------------------
         do j = 1, nb
            ...
            if (j .le. n) then
               do l = locA(j), locA(j+1)-1
                  i = indA(l)
                  Acol(l) = Acol(l) * ( cScale / scales(n+i) )
               end do
            end if
            x(j) = x(j) / cScale
         ...
         call ddscl ( m, scales(n+1), 1, pi, 1 )
         if (iObj  .gt. 0) scaleObj = scales(n+iObj)
...
!        ---------------------------------------------------------------
         do j = 1, nb
            ...
            if (j .le. n) then
               do l = locA(j), locA(j+1)-1
                  i = indA(l)
                  Acol(l) = Acol(l) * ( scales(n+i) / cScale )
               end do
            end if
            x(j) = x(j) * cScale
            if (bl(j) .gt. -bplus)  bl(j) = bl(j) * cScale
         ...
         call dddiv ( m, scales(n+1), 1, pi, 1 )
         scaleObj = one
      end if

      end ! subroutine s2applyScales

!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

      subroutine s2unpack
     &   ( jq, m, n, neA, ColNorm, nlocA, locA, indA, Acol, y )

      implicit
     &     none
      integer
     &     jq, m, n, neA, nlocA, locA(nlocA), indA(neA)
      double precision
     &     ColNorm, Acol(neA), y(m)

!     ==================================================================
!     s2unpack  expands the jq-th column of  ( A  -I )  into  y.
!
!     21 Jun 2004: Norm of unpacked column computed.
!     21 Jun 2004: Current version of s2unpack
!     ==================================================================
      integer
     &     i, islack, l
!     ------------------------------------------------------------------
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      subroutine s2dmat( matfil, n, nb, neA, nlocA,
     &                   Acol, indA, locA, hs )

      implicit           none
      integer            matfil, n, nb, neA, nlocA
      double precision   Acol(neA)
      integer            indA(neA), locA(nlocA), hs(nb)
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      subroutine s2xmat
     &   ( matfil, n, nb, neA, nlocA, locA, indA, Acol, hs )

      implicit
     &     none
      integer
     &     matfil, n, nb, neA, nlocA, locA(nlocA), indA(neA), hs(nb)
      double precision
     &     Acol(neA)
