...
*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     NeedLU, NewLU, NewB
      integer
     &     iExit, iObj, itn, leniw, lenrw, LUrequest, m, mBS, n, nb,
     &     neA, nlocA, nnL, nrhs0, nrhs, nS, nSwap, printLevel, typeLU,
     &     locA(nlocA), indA(neA), hs(nb), kBS(mBS),
     &     iy(nb), iy1(nb), iw(leniw)
      double precision
     &     Acol(neA), rhs(nrhs0), bl(nb), bu(nb), x(nb),
...
!     18 Nov 2001: printLevel added as parameter to s2sing.
...
!     Umin   is the smallest diagonal of U after last BS factor.
      ...
      if (NeedLU) then
         ...
         if (Prtlvl10) then
            write(str, 1005) iw(nFac), LUrequest, itn
...
!           ------------------------------------------------------------
            ...
            call s2BLU
     &         ( inform, BS, printLevel, m, n, nb, nBS,
     &           neA, nlocA, locA, indA, Acol,
     ...
     &         ( m, mBS, nb, nS, hs, iw(iP), kBS, iy, iw(locr), nSwap )
            ...
            if (Singular  .and. .not. BRdone) then
               BRfac  = .true.
               Ptask  = BR
            end if
         end if ! m basics
...
!     Report file 91, 92, 93 activates this call
            ...
            matfil = iw(130)    ! Report file n
            if (matfil .gt. 0) then
               call s2dmat
     &            ( matfil, n, nb, neA, nlocA, Acol, indA, locA, hs )
            end if

            call s2BLU
     &         ( inform, Btask, printLevel, m, n, nb, nBS,
     &           neA, nlocA, locA, indA, Acol,
     &           kBS, iw(iP), rw(LUa), iw(indc), iw(indr), lenaLU,
     &           iy, iy1, y, iw, leniw, rw, lenrw )
...
!
!     On exit,  nonlin and nSlack are undefined if start = 0.
!
...
!!!   minA   = 6 * min( m, n ) * necolA  ! Too little for BSfac
      ...
      iw(373) = indc      !
      iw(374) = indr      !
      iw(375) = cols      ! 3 new work arrays for lu1mxr
      iw(376) = markc     !
      iw(377) = markr     !
...
*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      subroutine s2BLU
     &   ( iExit, Task, printLevel, m, n, nb, nBS,
     &     neA, nlocA, locA, indA, Acol,
     ...
     &     none
      integer
     &     Task, iExit, printLevel, m, n, nb, nBS, neA, nlocA, lenaLU,
     &     leniw, lenrw, locA(nlocA), indA(neA), kBS(nBS), iP(nBS),
     &     indc(lenaLU), indr(lenaLU),
     &     iy(nb), iy1(nb), iw(leniw)
      double precision
     &     Acol(neA), aLU(lenaLU), y(nb), rw(lenrw)
...
!        y is an output parameter, used by s2sing.
     ...
     &        iw(iPloc), iw(iQloc), iy, iy1,
     &        iw(cols),  iw(markc), iw(markr), y, iExit )
...
!     18 Nov 2001: printLevel added as parameter.
...
!           Make the appropriate slack basic.
            ...
            hs(n+i) = 3
            nSing   = nSing + 1
            if (printLevel .ge. 10  .and.  nSing .le. nPrint) then
               write(str, 1000) j, i
               call snPRNT( 3, str, iw, leniw )
            end if
         end if
      ...
      if (printLevel .ge. 10  .and.  nSing .gt. nPrint) then
...
!     ----
!     DefTol   Set default tols for current TP (TPP, TRP or TCP)
!     ResetTol Set default tols for TP specified by user.
!     RedTol   Reduce tols or switch from TPP to TRP.
!     MinTol   Set minimum tols for current TP.
!
...
!     31 Jul 2003: snPRNT adopted.
!     09 May 2014: Chanaged RstTol to ResetTol.
!     =================================================================
      character
     &     str*72
      integer
...
!     ------------------------------------------------------------------
      ...
      integer            DefTol,   RedTol,   MinTol,   ResetTol
      parameter         (DefTol=0, RedTol=1, MinTol=2, ResetTol=3)
...
*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      subroutine s2tryLU
     &   ( itn, LUrequest0, nS, LUrequest, LUok, typeLU,
     &     iw, leniw, rw, lenrw )
     ...
     &     LUok
      integer
     &     itn, LUrequest0, LUrequest, nS, typeLU, leniw, lenrw,
     &     iw(leniw)
      double precision
     &     rw(lenrw)
...
!        LUrequest indicates the reason for the LU request.
!
...
!        Try to find a better basis.
      ...
      else if (LUmod .gt. 0 .and.
     &        (LUmod .ne. 1 .or.  LUrequest .ne. 5)) then

!        The LU has been modified since the last factorize.
!        Refactorize with the current LU tolerances. (The LU tolerances
!        are reduced if the first LU update gave a singular Basis.)
