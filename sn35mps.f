...
*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     none
      integer
     &     iExit, iMPS, maxm, maxn, maxneA, nnCon, nnJac, nnObj, m,
     &      mincw, miniw, minrw, n, neA, iObj, nS, lencw, leniw, lenrw,
     &     indA(maxneA), hs(maxm+maxn), locA(maxn+1), iw(leniw)
      double precision
     &     ObjAdd, Acol(maxneA), bl(maxm+maxn), bu(maxm+maxn),
...
*            section of the MPS file.
*
*     maxneA is an overestimate of the number of elements (matrix
...
*
*     neA     is the number of matrix coefficients in COLUMNS section.
*
...
*             respectively.
*
*     Acol(*) contains the neA entries for each column of the matrix
...
*            = 112  if maxm, maxn or maxneA were too small.
...
*     ------------------------------------------------------------------
      ...
      call s3mps
     &   ( iExit, key,
     &     maxm, maxn, maxneA, lenh, nnL, nnCon, nnJac,
     &     m, n, neA, negCon, nS,
     &     iObj, ObjAdd,
...
*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      subroutine MPSout
     &   ( iMPS, m, n, neA, nName, PrbNms,
     &     Acol, indA, locA, bl, bu, Names )

      implicit
     &     none
      integer
     &     iMPS, nName, m, n, neA, indA(neA), locA(n+1)
      double precision
     &     Acol(neA), bl(n+m), bu(n+m)
      character
...
*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     mincw, miniw, minrw, m, n, neA, nextcw, nextiw, nextrw,
...
*                objective function.
*
*     m, n, neA  are the number of rows, columns and elements in A.
*
...
*     Allocate space for the arguments of sqopt and s5solve.
*     These are for the data,
*        locA, indA, Acol, bl, bu, Names
*     and for the solution
*        hs, x, pi, rc, hs.
*     The true values of m, n and neA are also found.
...
*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     none
      integer
     &     iExit, lenh, m, mincw, miniw, minrw, n, neA,
...
*     ==================================================================
      integer
     &     nb, lNames, lindA, llocA, leType, lhs, lAcol, lbl, lbu,
     &     lx, lpi, lrc, lfeasType, lkBS, lkeynm
*     ------------------------------------------------------------------
*     Allocate space for the data,
*              locA, indA, Acol, eType, bl, bu, Names
*     and for the solution
*              hs, x, pi, rc, hs.
...
*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     nnL, nnCon, nnJac, m, n, neA, negCon, nS, keynam(lenh),
     &     kBS(maxm+maxn), indA(maxne), hs(maxm+maxn), RowTypes(maxm),
...
*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     lrow, lennm, lenh, nnL, nnCon, RowTypes(maxm),
...
*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     m, maxn, maxneA, n, neA, negCon, ncoll, nnL, nnCon, nnJac,
     &     RowTypes(m), indA(maxneA), locA(maxn+1), kBS(m), iEr(20),
     &     ncard(6), keynam(lenh), iw(leniw)
      double precision
     &     ObjAdd, Acol(maxneA), bl(maxn+m), bu(maxn+m), pi(m),
     &     rw(lenrw)
      character
     &     key*4, Names(lennm)*8, cw(lencw)*8
...
*     ==================================================================
     ...
     &     lJac, lDenJ, MPSerr, mLst, mRhs, mRng, nA0, neA1
      double precision
...
*     Or too many columns or elements?
      ...
      else if (neA .gt. maxneA) then
         write(str, 3050) maxneA, neA
         call snPRNT( 3, str, iw, leniw )
         iEr(2) = iEr(2) + 1
         iExit  = 112
         return
